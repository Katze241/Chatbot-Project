...
# LLM 정의 (Ollama 사용) - 메모리 효율적인 모델 사용
llm_cheap = Ollama(model="llama3:8b")  # 사용 가능한 모델
llm_powerful = Ollama(model="qwen2:7b")  # 사용 가능한 모델
...
# 성능 최적화를 위한 상수
MAX_CONTEXT_LENGTH = 8000  # 컨텍스트 최대 길이
MAX_SENTENCES = 20  # 최대 문장 수
MAX_WORKERS = 3  # 병렬 처리 최대 워커 수
CACHE_SIZE = 1000  # 캐시 크기
MIN_CONTEXT_QUALITY_SCORE = 0.3  # 최소 컨텍스트 품질 점수
...
def calculate_context_quality_score(query: str, context: str) -> float:
    """
    컨텍스트의 품질을 평가하는 함수
    키워드 매칭, 길이, 구조 등을 고려하여 점수를 계산합니다.
    """
    if not context or not context.strip():
        return 0.0
    
    # 키워드 추출
    query_keywords = extract_proper_nouns(query, max_keywords=5) + extract_keywords_from_text(query, max_keywords=10)
    context_keywords = extract_keywords_from_text(context, max_keywords=20)
    
    # 키워드 매칭 점수 계산
    keyword_matches = sum(1 for qk in query_keywords if any(ck.lower() == qk.lower() for ck in context_keywords))
    keyword_score = keyword_matches / max(len(query_keywords), 1)
    
    # 길이 점수 (너무 짧거나 긴 컨텍스트는 낮은 점수)
    length_score = min(len(context) / 1000, 1.0)  # 1000자 기준
    
    # 구조 점수 (문장 수, 단락 구분 등)
    sentences = re.split(r'[.!?]+', context)
    sentence_count = len([s for s in sentences if len(s.strip()) > 10])
    structure_score = min(sentence_count / 10, 1.0)  # 10문장 기준
    
    # 종합 점수 계산
    total_score = (keyword_score * 0.5 + length_score * 0.3 + structure_score * 0.2)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CHROMA_DB_PATH) or not os.path.exists(LARGE_CHUNK_PICKLE):
        ...
        query = "기술 지원팀이 7월 10일까지 긴급 패치를 배포해야 하는 이유는 무엇입니까?"
